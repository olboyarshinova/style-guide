...
- Используйте семантические теги: ```<button>, <nav>, <main>, <article>, <section>, <header>, <footer>```.
...
- Используйте `trackBy` при использовании `*ngFor`.

Для вывода в шаблоне коллекций (списки, строки таблиц и т.п.), которые могут быть динамически изменены (например,
добавление/удаление элемента), `trackBy` обязателен к использованию. С помощью `trackBy` Angular может определить,
какие именно элементы коллекции необходимо перерисовать в шаблоне. Без `trackBy` при изменении коллекции она будет
перерисована в шаблоне полностью, что может негативно сказаться на скорости работы интерфейса.
